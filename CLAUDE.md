...
| **"GA4" / "GA4 수집" / "GA4 연결" / "citymarket_arrival" / "가격확인율" / "UTM 슬러그 매핑" / GA4 탭·수집·소비 수식 작업** | **★ 별도 GA4 전담 task로 진행 (2026-07-16 종민 결정).** 인수인계 정본 = `_docs/GA4_TASK_시작프롬프트.md` (연결방식·현행구조·탭구조·트리거·검증법·함정·우선과제 P1~P4). 새 세션 시작 명령: `_docs/GA4_TASK_시작프롬프트.md 읽고 GA4 task 시작. 먼저 현황 진단부터.` 소유영역=GA4 코드만(타 task 파일 ❌) |
