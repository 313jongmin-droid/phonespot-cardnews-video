...
## 3단계 — 렌더 매칭을 임베딩으로 교체 (적용됨)

- `shorts/scripts/codex_semantic_visual_match.py` (영상에 실제로 박히는 그림을 고르는 매처).
- 임베딩 모델이 켜져 있으면(`ce.available()`), 청크 문맥과 ① 소스 이미지 설명(prompt.md) ② 라이브러리 일러스트
  개념을 **의미 코사인**으로 비교해 가장 가까운 걸 고른다. 모델 없으면 기존 lexical 점수로 폴백(동일 로직, 임계만 전환).
- **보너스:** 다국어 임베딩이라 **한국어 캡션 ↔ 영어 prompt.md 설명**이 의미로 매칭된다(버그2 근본원인 4 해결 → 소스 이미지 배치 정확도↑).
- 임계: `EMBED_MIN_IMAGE = 0.42`, `EMBED_MIN_ILLUST = 0.42` (코사인 0~1). **이 두 값만 조절**하면 됨:
  올리면 더 엄격(무관 매칭↓, 대신 중립 필러↑) / 내리면 더 관대.
- 검증: 임베딩 모드·lexical 폴백 둘 다 "보안 잠금 설정" 청크를 `security_feature_lock`에 정확 매칭(단위 테스트).
- 백업: `shorts/scripts/codex_semantic_visual_match.py.bak_embed_match_*`
- **주의(미실측):** 샌드박스는 모델 다운로드가 막혀 실제 코사인 분포는 PC에서 확인해야 함. 영상 1편 렌더해서
  그림이 본문과 맞는지 육안 확인 → 너무 자주 중립 필러로 빠지면 임계를 0.35~0.40으로 낮추고, 무관 매칭이 보이면 0.45~0.5로 올린다.
