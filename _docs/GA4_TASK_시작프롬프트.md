...
> **✅ 2026-07-21 확인 — 7/14 자가복구.** 7/3~7/13 전멸(7/6만 1) 후 7/14부터 재개(140~290/일, 단 배포전 500~870/일의 ~1/3). 잔여 = ① 시티마켓 카톡 버튼은 있으나 GTM 이벤트 태그(`kakao_chat_click`) 없어 시티마켓 카톡 0, ② 7/9 시티마켓 개편 후 랜딩이 `(not set)`으로 잡히는 계측 저하 — 둘 다 사이트/GTM(`GTM-TMXR6VL9`) 몫.
...
> **✅ 2026-07-21 확인 — 오염 없음.** KT `_설정` `GA4_PROP_ID=543861861` 정상 반영. 폰스팟 `534396517`은 KT 시트 전체에 0건, KT GA4_자동 캠페인도 kt_iphone/kt_galaxy 등 KT 고유(폰스팟 캠페인 누출 0). **단 KT 실버그 미해결**: KT는 카톡클릭 이벤트명이 `chat_click`인데 소비 수식(`Code.js:975`)은 `kakao_chat_click` 기대 → KT 카톡전환율 0으로 잡힘. KT엔 `citymarket_arrival` 자체가 없어(홈페이지 랜딩) 가격확인율도 원천 0.
...
> **✅ 2026-07-21 구현됨 (별도 탭 방식).** 신규 `apps_script/page-funnel.js` + 탭 `페이지별_퍼널`(요약)·`GA4_페이지별`(일별). **세션 시작 페이지(landingPage) 기준**으로 리틀리/시티마켓/기타 분리 — 리틀리 경유 시티마켓 도착·카톡은 리틀리 성과로 귀속. GA4_자동에 차원을 넣지 않고 **신규 탭만 생성**해 기존 SUMIFS·대시보드·리틀리 무손상(컬럼시프트 회피). 트리거 `setupPageFunnelTrigger` 매일 03:30. `_설정` 키 `PF_A/B_NAME`·`PF_A/B_PATHS`·`PF_NOTSET_BUCKET`로 브랜드별 조정(코드 push 불필요). **함정·상세 = `_docs/SYSTEM_MAP.md` G단원 "2026-07-21 세션"**(6분한도→landingPage 경로만+setValues 배치, pf_dateSet_ 날짜문자열 비교, 시티마켓 카톡=GTM 태그 필요, (not set) 랜딩계측 이슈).
