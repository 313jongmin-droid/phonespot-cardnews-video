...
> **★ 후보 리스트업 규칙 (2026-06-22)**: ① **모든 후보는 사실검색·검증 완료 후에만 표에 올린다** — "사실확인 필요"·제목만 본 미검증 항목 등재 금지(특정 사건/뉴스는 WebSearch로 실체·시점·매장정합 확인, 부적합이면 제외 근거 1줄). ② **각 후보에 한 줄 요약 병기** — 토픽 옆에 "무슨 청크/스크립트가 될지"(핵심 메시지 + 카드 흐름) 1줄 미리보기.
