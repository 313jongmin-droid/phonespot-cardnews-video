...
- 2026-06-22 (대시보드 총점검 보강 — 리틀리+실비용): 전체 33탭 인벤토리 결과 통합대시보드 미반영분 식별(리틀리 유입/결제내역 실비용/네이버SEO/실매출). 1차로 updateLitlyAndPaymentSections_(Code.js) 추가 — 옛 여백 45~59행을 두 섹션으로 채움(여백문제도 동시 해결). (A)리틀리 유입(45~49): 어제/7일/30일 방문자수(SUMIFS 리틀리 B)·클릭수(C)·CTR + 최신 유입경로비율(G열 마지막 비공백, FILTER/INDEX). (B)실비용 대조(51~59): 채널별 이번달 카드결제(결제내역 D, 구분=채널, 날짜 이번달) vs API광고비(채널시트 지출, KPI ADS 매핑과 동일: 메타_통합H/네이버_통합H/구글G/카카오G/당근G) + 차이 + 합계. nightlyDashboard·refreshAll에 호출 추가. recordLastRefresh_는 숨김/클리어 제거(섹션이 채움)·시각 A7 유지. 스타일=기존 섹션과 통일(진청 헤더/연청 컬럼/테두리/원·% 포맷). 적용=nightlyDashboard 1회. 미반영 잔여(SEO·실매출)=3·4번 보류.

- 2026-06-22 (통합대시보드 V2 재설계, 라이트톤): buildDashboardV2()(Code.js) 신설 — 단일 빌드함수로 전체 재배치. ① 요약 스트립 1~2행(이번달광고비/30일CPL/30일개통률/어제문의/출처미상%) setFrozenRows(2). ② 기간별 핵심(4~8) ③ 채널별 효율(10~16, I열 드롭다운) ④ 리틀리 유입(18~22) ⑤ 실비용 대조(24~31) ⑥ SNS(33~38) ⑦ 푸터 시각 + 39~59 hideRows. 라이트톤(섹션헤더 #EEF1F5/컬럼헤더 #F7F7F8, 진청 풀배경 폐기), 전 섹션 A~F 6컬럼 통일. 빈 카톡 리포트 제거(개통은 기간별핵심에 흡수). 수식은 기존 updateKPISummary/ChannelMatrix/SNSReport/Litly·Payment에서 그대로 추출 재사용. nightlyDashboard·refreshAll이 이제 buildDashboardV2 호출(기존 update* 함수는 정의 유지=폴백). recordLastRefresh_=시각만 반환(대시보드 미기록). 광고그룹추이(adgroup-trend.js 60행)는 미변경. ROAS는 매출원천 없어 개통률로 대체(라벨 정직). 적용=buildDashboardV2 1회 실행. 다른 시트 톤 통일=후속(task).

- 2026-06-22 (대시보드 V2 마무리·결정사항): buildDashboardV2 후속 수정 — ① 클리어 범위 A1:Z140→A1:Z59 (광고그룹 추이 60행~ 보존; 처음엔 140까지 지워 추이 1회 소실 → setupAdgroupTrendChart 재실행으로 복구). ② 행높이 균일화 setRowHeights(1,59,22)+요약값행 34px (들쭉날쭉 해결). ③ 옛 드롭다운(E16/E28/E36) clearDataValidations() (E16 데이터확인 충돌 토스트 해결). ★ 결정: 격자선 숨김/라이트테두리/스트립배경 폴리시는 사장님이 **미적용**(격자선 유지) → 되돌림. HTML 웹앱 대시보드는 로딩 느려서 **비채택**(시트 네이티브 유지). 통합대시보드 V2 = 현행 확정. 후속 보류: 다른 시트 톤 통일(task), 출처미상 85% 입력정착, 실매출/네이버SEO 섹션.
