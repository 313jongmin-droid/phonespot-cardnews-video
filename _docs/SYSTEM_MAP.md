...
- 2026-06-18 (광고운영 기능확장 STEP1-3, 종민 로드맵 승인분): **데이터 정합성 + 스케줄 + 알림 (G단원).** **STEP1 UTM 정합성**: ① `Code.js refreshUtmSlugDropdowns()`(GA4_자동 실측 utm_campaign을 UTM_매핑 C열에 채널별 드롭다운, 오타/슬러그 불일치 차단) + 메뉴 🏷️. ② named range `UTM_CH`(A열)·`UTM_KEYVAL`(B:C) = `ensureUtmNamedRanges_()`(idempotent, sync/refreshAll 시작 시 자동 생성) + 메뉴 🔗 + `setupUtmNamedRanges()`. 메타(`meta-sync.js:329`)·네이버(`naver-synce.js:412`)·당근(`danggn-sync.js:280`) 수식을 `FILTER('UTM_매핑'!B:C,A:A=…)` → **`FILTER(UTM_KEYVAL, UTM_CH=…)`** 전환 = 컬럼 삽입/이동 시 명명범위 자동추적 → 6-17 마이그레이션 시프트 사고 구조적 차단(P2-5+6 통합). **STEP2 스케줄**: refreshAll 트리거 안전화(`const ui` → `let ui=null; try getUi catch` + `if(ui)` 가드 + logSync_) → 야간 트리거 가능. `setupRefreshAllTrigger()` 매일 02:45(개별 sync 01:30~02:30 다음) + 메뉴 ⏰. `adgroup-trend.js setupAdgroupTrendTrigger()` 매일 02:50 + refreshAdgroupTrendChart ui 가드. **STEP3 알림(신설 `alerts.js`)**: `tgSend_`(텔레그램, Script Property TELEGRAM_BOT_TOKEN+TELEGRAM_CHAT_ID 필요, 미등록 시 스킵) / `runHealthCheck_`(메타·네이버·당근 통합 최신일·GA4세션 전행0 감지 = 침묵실패 포착, refreshAll 끝에 호출) / `checkAdTargets_`(어제 CPL>TARGET_CPL 기본5만원 or 지출30000+문의0 경고) / `sendMorningBriefing`(매일 09:00 KPI 브리핑) + `setupMorningBriefingTrigger` + `testTelegram` + 메뉴 🔔. Code.js onOpen에 `buildAlertsMenu_` 등록. **검증**: 6파일 전부 node --check PASS, 각 치환 count==1, 옛 `FILTER('UTM_매핑'!B:C` 0건. **편집법**: Code.js(46KB)·meta-sync(83KB)는 bash-python(마운트 정상 확인 후), 신규 alerts.js는 Write. **★ 배포 후 활성화 필수(메뉴 1회씩)**: STEP1 🏷️드롭다운+🔗named range / STEP2 ⏰02:45+⏰02:50 트리거 / STEP3 TELEGRAM_BOT_TOKEN·TELEGRAM_CHAT_ID Script Property 등록→🔑테스트→⏰09:00 트리거. (TARGET_CPL 조정은 Script Property.) **보류**: STEP4(크리에이티브 폐루프 P3-8/9/10)·STEP5(자연어질의 P3-12) = 종민 결정 'STEP1-3 먼저 배포·검증 후'. Part3 보류=카카오/구글/틱톡 광고·멀티브랜드. 상세 로드맵 = `outputs/광고운영_점검+기능로드맵_2026-06-18.md`. push = apps_script 6파일.
- 2026-06-18: **A·E단원 — 이미지 가져오기 '제안 생성' Failed to fetch 수정(graceful 폴백, 종민 선택).** 원인=CLIP 모델 캐시 없는 PC서 fastembed 모델로드가 네트워크 다운로드로 무한대기(try/except는 행 못잡음)→핸들러 subprocess.run(timeout無)→fetch 무응답. 수정 2파일: `codex_image_embed.py`(_call_with_timeout 스레드 타임아웃 25s+HF 다운로드 타임아웃→멈추면 폴백) + `server.py`(video/card_import_propose subprocess timeout=120+TimeoutExpired 백스톱). py_compile·import 0.35s·available()즉시False 검증. **반영: 패널 재시작**(server.py)·`시스템 업로드`로 부사수 PC 전파. E단원 함정에 상세.
- 2026-06-18 (광고운영 기능확장 STEP4-5): **크리에이티브 인사이트 + 자연어 질의 (G단원).** **STEP4 신설 `creative_insights.js`**: `analyzeCreativePerformance`(메타_소재 노출500↑ CTR 랭킹 → '소재_성과' 시트 승자5/손실3 + 텔레그램, P3-8 A/B) / `syncBenchmarkScheduled`(Script Property BENCHMARK_TERMS 콤마구분, 기본 휴대폰성지·갤럭시S26·자급제·핸드폰싸게 → `syncMetaAdLibrary(term,'BM')`) + `setupBenchmarkTrigger`(매주 월 04:00) + 메뉴 🎨 소재 인사이트. **P3-10 폐루프 = 데이터측만**: 소재_성과 시트 생산. generator.html이 직접 읽는 완전연결은 별도(데이터계약 변경 신중 — `광고_라이브러리` 시트 평가/CTR 컬럼 `pushLibraryFromGenerator`가 연동지점). **STEP5 신설 `nl_query.js`**: `askSheetQuestion`(ui.prompt → 통합대시보드+채널통합 최근7행+소재_성과 컨텍스트 → Gemini gemini-2.5-flash, 추측금지 프롬프트 → ui.alert) + `_geminiAsk_`/`_buildSheetContext_` + 메뉴 🤖 데이터 질문. GEMINI_API_KEY 기존 등록분 재사용. Code.js onOpen에 buildCreativeInsightsMenu_·buildNlQueryMenu_ 등록. **검증**: 전 apps_script .js 11개 node --check PASS. generator.html 미접촉. **활성화**: 🎨→🏆 소재 A/B 평가 1회 + ⏰ 벤치마크 주간 트리거 / 🤖→💬 질문(GEMINI_API_KEY 이미 있음). 선택: BENCHMARK_TERMS Script Property. push = Code.js + creative_insights.js + nl_query.js (STEP1-3는 이미 별도 배포됨). 로드맵 = `outputs/광고운영_점검+기능로드맵_2026-06-18.md`.
- 2026-06-18 (promo 타이포 트랙 정비 + 자가개선 Phase 0·2a — C단원): ① 나레이션 모순 정정(GUIDE_TYPOGRAPHY: promo=효과음+음악만, tts=비렌더 메모) ② 음악 fallback 안전화(`promo_pick_music.py`: 무드 곡 없으면 정상무드만 fallback, sting/규칙위반 제외) ③ MD↔JSON 정합(`promo_check_sync.py` + `promo_md2json.py` `__main__` 가드) ④ 후킹 학습신호(review 12편 `- 후킹:` 채움, 초안분류) ⑤ `promo_manifest.py` `variant_id`(옛 행 자동 백필) ⑥ 성과 귀속=관리시트 `유튜브`/`인스타` 운영일지 조회수·좋아요(업로드=사람, 비고=outfile 조인키), `_results.csv`+`promo_score.py`(축 풀링+게이트)+`promo_results_add.py` ⑦ 생성 근거 패널 `promo_plan.py` ⑧ 설계서 `promo/AUTOLOOP_DESIGN.md` 신설. **검증**: manifest 마이그레이션·score 조인/풀링·check_sync 드리프트탐지 격리테스트 통과(단 bash 마운트가 `promo_manifest.py`를 truncate read → file-tool/PC로 검증). 테스트=종민 내일. 상세 = C단원 'promo 자가개선' 블록 + `promo/AUTOLOOP_DESIGN.md`.
