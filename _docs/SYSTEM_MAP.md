...
- `insta+30%` 인스타 Top 게시물·**릴스**(도달·조회·저장) 주제/포맷 매칭 / `insta+20%` 잘 되는 릴스 포맷 적용 — 소스=`인스타` 시트(Drive 스냅샷 직접 참조, 인스타_인사이트 MD 자동화는 추후) (★ 2026-06-19 사장님: 콘텐츠 수집은 유튜브+인스타 성과 기준)
...
- 2026-06-19 (수집 성과 기준 재정의 — B단원, 사장님): 콘텐츠 수집 후보 가중치 = **유튜브 영상 조회수(`유튜브_인사이트`) + 인스타 게시물·릴스 성과(`인스타` 시트/Drive 스냅샷)** 기준. **메타(광고)는 수집에서 제외** = 광고 카피용(G단원)이지 발행 주제 선별용 아님. 가중치 매트릭스 `meta` → `insta`(insta+30% Top게시물·릴스 / insta+20% 릴스 포맷). 갱신처: SYSTEM_MAP B 가중치, INSTRUCTIONS 사전점검, PREFLIGHT. **미구축**: 인스타_인사이트 MD 자동생성(유튜브_인사이트 같은) 없음 → 당장은 `인스타` 시트 직접 참조, 추후 새 clasp 파일로 자동화 권장. **CLAUDE.md STEP1 #8(메타_인사이트)도 인스타로 갱신 필요 = 종민(92KB Edit 위험).**
