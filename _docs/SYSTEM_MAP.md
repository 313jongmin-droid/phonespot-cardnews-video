...
- 2026-06-15: **F단원 머신 모델 갱신 (G단원도 일관) — 노트북 → 로컬 PC push 주체 변경.** 사장님 운영 = 노트북은 크롬 원격 입구만, 실제 작업/git push는 로컬 PC. 머신 역할 표 4행 + 원칙 모두 갱신. Git for Windows 2.54.0 설치 사실 박음 (Phase 1 셋업 시). 광고운영 트랙 자동 배포 흐름(clasp push --force, **콘솔 직접 수정 ❌**) 명시. 정본 = `CLAUDE.md` STEP 0.
- 2026-06-15: **GitHub Actions 실패 알림 셋업 (G단원).** `deploy-apps-script.yml`에 `Notify Telegram on failure` step + GitHub Secrets `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID`. 카드뉴스용 봇 재활용. 정본 = `ads/MULTI_BRAND_ARCHITECTURE.md`.
- 2026-06-15: **`_secrets/` 백업 안 함 결정 + 재발급 절차 가이드 (DISASTER_RECOVERY.md 갱신).** 사장님 결정 = 별도 백업 비채택. **★ 핵심**: Apps Script PropertiesService 키는 Google 클라우드 저장 = `_secrets/` 손실 무영향. 영향 = 로컬 텔레그램 listener 정도. 정본 = `_docs/DISASTER_RECOVERY.md`.
