...
- 2026-06-23 (세션): **★ 시트 탭 이름 단축 — `_통합`→`+`, `UTM_매핑`→`UTM` (G단원 전반, 사장님 결정).** 매핑: **메타_통합→메타+ / 네이버_통합→네이버+ / 당근_통합→당근+ / 구글_통합→구글+ / UTM_매핑→UTM**. **당근_UTM_매핑은 유지**(별도 매핑 시트, 건드리지 않음). 라이브 코드 `apps_script/` + `apps_script_sheet_export/` 문자열 일괄 치환(메타29·네이버26·당근24·구글15·UTM 42 = 약 136곳). **치환 안전장치**: ① `당근_UTM_매핑` 먼저 placeholder 보호 후 `UTM_매핑`→`UTM` 치환 → 복원(정규식상 `당근_UTM_매핑`이 `UTM_매핑`으로 접혀 들어가 오염 위험) ② `통합대시보드`(27곳)는 풀토큰만 치환해 무영향. 실제 탭 리네임 = 일회용 `renameTabsToPlus_()`(`Code.js` 말미, getSheetByName 가드+이미존재 스킵). **★ 실행 순서(다운타임 회피)**: push→배포(1~2분)→**배포 직후 `renameTabsToPlus_()` 1회 실행**→`buildDashboardV2`(또는 새로고침) 1회. 코드가 새 이름을 먼저 배포되면 옛 탭(메타_통합) 참조 실패하므로 리네임을 즉시 이어서 실행(야간 트리거 1~3시라 낮 작업이면 충돌 0). **시트 안 수식은 탭 리네임 시 구글시트가 자동 갱신**(코드 문자열만 수동 치환 필요했던 것). **★ 함정**: 이 맵·`ads/` 가이드 본문의 옛 이름(`메타_통합`!H 지출 등) 표기는 점진 갱신 — 코드는 새 이름이 정본. 향후 시트명 언급 시 **메타+/네이버+/구글+/당근+/UTM** 사용. `ads/code/`(레거시 미배포)는 미치환(필요 시 일관성용 후속).
- 2026-06-23 (세션): **★ CI 텔레그램 알림 단일화 — 모든 push에 완료 알림 1개 (G단원 인프라).** 사장님 요청: push 즉시 알림 끄고 "완료" 1개만, 폴더 전체 적용. ① `notify-push.yml`(모든 push에 "Git push 완료" 발신) **완전 삭제**(9159053). ② `deploy-apps-script.yml` → `Deploy & Notify`로 재구성: 트리거를 `apps_script/**` 한정에서 **모든 push(`on: push: branches:[main]`)**로 확장 + `dorny/paths-filter@v3`로 apps_script 변경 여부 판정 → 배포 단계 전부 `if: steps.changes.outputs.appsscript == 'true'` 조건부. 완료 알림 1개: `if: success()`에서 apps_script 변경이면 "✅ Apps Script 배포 완료", 아니면 "✅ push 반영 완료(코드 외 변경)"; `if: failure()`는 실패 시만. **결과: 한 번 push = 정확히 1개.** "2번 왔다"의 실제 원인 = 연속 2회 push(각 apps_script 변경 → 각 배포 1개), 중복 버그 아님. **★ 함정/교훈**: `.github/` 경로는 bash 샌드박스에서 **쓰기 불가**(Operation not permitted) → 호스트 파일도구(Write/Edit)로 수정. 단 **bash 마운트는 폴더연결 시점 frozen** = 호스트 Write/Edit 후 bash `cat/wc/tail`이 **stale(잘린 듯) 표시** → 검증은 **Read 도구(호스트 fs)로** 해야 정확(이번에 bash는 110줄 잘림, Read는 121줄 완전). YAML `on:`은 py-yaml에서 `True` 키로 파싱되지만 GitHub은 정상 처리(무해).
- 2026-06-23 (세션): **협업 채널 2탭 → 1탭 병합 '협업 리스트업' (G단원, `apps_script/collab-listup.js` 신설).** 부사수가 나눠둔 `채널 리스트업`(후보 마스터: 채널명·플랫폼·링크·카테고리·MCN·연락처·팔로워·조회수·좋아요·댓글·참여율·콘텐츠형식·적합도·시청자층·비고)과 `협업메일 현황`(발송/회신 추적, 채널명만 적으면 리스트업에서 수식 자동복사)을 1탭으로 합침. 함수 **`buildCollabListup()`**(언더스코어 없음=드롭다운 노출): ① `협업 리스트업` 생성/초기화 ② 마스터 14열 + 추적 3열(발송일·회신일·발송경로) + 비고 = 18열, 헤더명 기준 매핑(컬럼이동 내성) ③ 협업현황에 실입력된 발송/회신 있으면 채널명 매칭 이관(현재 전부 공란=손실0) ④ 우측 요약(총 발송·회신·회신율) 자동집계 ⑤ 적합도 조건부색(상=초록/중=노랑/하=회색) ⑥ 원본 2탭 → 이름 ' (구)' + `hideSheet()`(되돌리기 가능, 사장님 선택). **★ 함정**: (a) 1행 제목을 가로 전체병합(A1:R1)한 상태에서 `setFrozenColumns(1)` 호출 시 "병합 셀 일부만 고정 불가" 에러 → **첫 열 고정 제거**(행 고정 setFrozenRows(2)만 유지). (b) 시트 정본 = 스프레드시트 `폰스팟 광고운영 관리대장`(bound script), Drive 스냅샷에 `채널 리스트업.json`/`협업메일 현황.json`로 백업됨. (c) 병합 후엔 탭간 수식링크 없어지고 한 행에 직접 입력하는 구조 — 발송일/회신일/발송경로만 입력하면 요약 자동집계.
