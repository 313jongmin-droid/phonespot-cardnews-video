...
- 2026-06-24 (세션, 생성기 범용화 — G단원, 사장님): **광고 생성기 폰스팟 하드코딩 → 폼 기반 브랜드 프로필 범용화 + 캐러셀 4컷.** ⚙설정 "🏷️ 브랜드 프로필" 폼(brandName/categories/copyEssence/sloganDiff/imageDomain/patternPool…)을 전역 `BP`로 빌더(buildCopyPrompt·슬로건·buildImagePrompt)에 주입 → 화장품/KT 코드수정 0. 기본=`폰스팟`=DEFAULT_BP 상속 = 출력 바이트동일(회귀안전, node 검증). 영속=localStorage `phonespot_brand_profiles` + 시트 `브랜드_설정`(Code.js `pushBrandProfilesToSheet`/`getBrandProfilesFromSheet`). 캐러셀=`generateCarousel`/`CAROUSEL_BEATS`/`buildCarouselSlidePrompt_`(후킹→오퍼→차별→클로징, 1:1 고정, 톤·AD_STYLE 4컷 통일). **★함정**: 백업 .v_*는 `apps_script_backups/`로(clasp rootDir="" 전체푸시 → 함수중복=배포깨짐). 상세 = `ads/IMPLEMENTATION_GUIDE_2026-06-09.md` §0-1 끝(2026-06-24 블록). 보류: 100%자율·판매까지(미정).
