...
- **★ promo_ai 품질·안전 룰 4종 확립 (2026-07-21 세션, 050 제작 중 종민 지적으로 확립). 정본 = `shorts/promo_ai/WORKFLOW.md` §5-1~5-3·§7**:
  ① **조명 일체화(§5-1)** — "미묘하게 AI 같다"의 원인 = 레퍼런스 인물이 스튜디오 정면광이라 **배경만 매장으로 바뀌어 합성 느낌**. 이미지 프롬프트에 `lit ONLY by the store's own neutral white LED ceiling lights / same light direction as background / NO studio lighting / contact shadow / deep depth of field(폰카 룩, 시네마틱 보케 금지) / rim light` 고정 삽입. **`warm store light` 금지**(주황에서 피부가 오렌지로 뭉개짐). **반례(재시도 금지)**: 그레인·노이즈·압축·비네팅=그냥 저화질(원본이 나음), 핸드헬드 흔들림·배속·트림=차이 미미, 색온도 후보정=구조 문제라 무효. **후처리로는 못 고침, 생성 단계에서 잡아야 함.**
  ② **나레이션 무음 패딩(§5-2)** — 슈퍼톤 mp3 **파일 자체에 앞뒤 무음 0.2~0.4초** 내장 → adelay 간격을 줄여도 0.6~0.9초 무음이 남음(050 v4=17.3초 중 26%가 무음). 합성 전 `silenceremove(앞) + areverse + silenceremove + areverse(뒤)` 필수. 볼륨은 `loudnorm=I=-14:TP=-1.5`(숏폼 표준, 미적용 시 −23dB대로 폰에서 작게 들림). 050 실측 17.3→14.97초, −23.8→−18.7dB. **004·005도 같은 패딩 있음**(재렌더 시 적용).
  ③ **부메랑 금지(§5-3)** — "다른 매장이 나쁘다" 프레임 **금지**(우리도 하는 것이면 자기 발등). 050 최초 기획 "매장 멘트=90% 거짓말"이 셋 다 해당(마지막 물량=512GB·1TB·특정색상 **실제 품절 흔함** / 오늘만 이 가격=성지 정책 **일 단위 실제 변동** / 요금제 유지=**우리도 거는 조건**) → 폐기. **조건의 존재 비난 ❌ → 숨기는 것 비난 ✅**, 폭로형 ❌ → **교육형("이것만 확인하면 됨") ✅**. 차별점은 "조건이 없다"가 아니라 **"먼저 공개한다"**. 리서치 근거(네이버 블로그 15건+): 판매점 콘텐츠는 전부 교육형, 공통 1순위 각도 = **"월 납부금 말고 할부원금"**. ★ **용어 통일: "실구매가" ❌ → "할부원금"**.
  ④ **산출물 정리·보고(§7)** — 확정 시 **덤프 5종 즉시 삭제**(sandbox 스크래치 `/mnt/outputs/b*`, `_chk*`·`_inv*`·`_rev*`·`_insp*` 검증폴더, 폐기본 mp4, 구버전 mp4(최종 1개만), 나레이션 임시 `2026-*.mp3`). 2026-07-21 1회 정리로 **약 1GB 회수**(outputs 812MB→12KB, out_promo_ai 272→42MB). `rm`이 `Operation not permitted`면 **`mcp__cowork__allow_cowork_file_delete`로 권한 받아 직접 삭제**(종민에게 탐색기 작업 떠넘기지 말 것). 완료 보고 시 **파일 present + 폴더 경로 + 파일 목록** 3종 필수.
- **★ 실사 AI 적합도 판별 정본 신설 = `shorts/promo_ai/LIVE_ACTION_FIT.md` (2026-07-21)**: TOPIC_ENGINE §4는 **라인 단위**(meme→실사) 매핑이라 같은 meme도 주제별로 적합도가 갈림(047 부적합 / 050 적합). **주제 단위 판별**을 위해 필수 3조건(1인칭 썰 재구성·표정 리액션·3~6컷 압축) + 가감점(기존컷 재활용+2·대사기반+2·CTA+1·무텍스트+1 / 체크리스트-3·표그래프-3·법령-2·신규장소-2) → `실사AI: 추천/조건부/비추천` 태그. 캘리브레이션 = 004·005·050(추천) vs 047(0점)·049(필수 미충족). **주제 task가 수집 시점에 태깅**하도록 §4에 복붙용 축약 구문 포함(주제엔진은 별도 task 소유라 참조만).
...
- 컴포넌트: `shorts/src/Cover.tsx`(`CoverShort`). 정적 1프레임. hook 헤드라인(`headline_lines`, 없으면 `video_title` 분할) + 폰스팟 브랜딩. **★ 히어로 이미지 제거 + 3변형 로테이션(2026-07-16)**: 라이브러리 그림 재사용·누적으로 커버들이 서로 비슷해지던 문제 → 히어로 일러스트/사진 완전 제거, **타이포·컬러만**. `VARIANTS[3]`(0=화이트 / 1=피치+하이라이트박스 / 2=오렌지반전)를 `variantIndex(slug)`=slug 앞 숫자 %3 으로 자동 배정 → 연속 슬러그가 화이트→피치→오렌지로 번갈아(그리드 다양성). 헤드라인 l1(본문)+l2(강조: text/chip). 구 `pickHeroSrc`/`visualSrc` 삭제.
...
- **★ CLIP 그림엔진(`codex_image_embed`)이 PC에 미설치면 content-gate 무력** → 텍스트/태그 매칭만으로 동작. 즉 "이름↔그림 불일치"의 범용 차단은 **CLIP 켜야 완전**. 안 켜진 동안은 정책 3(cpt_ 제외)+중립폴백+임계 0.48이 방어선. 검증 단서: `shorts/codex/image_embed_cache.json` 존재 여부. **★ CLIP 재활성화 + gemini 일러스트 태거 대체(2026-07-16)**: 런타임 Python 3.14 전환으로 fastembed가 사라져 CLIP·텍스트임베딩 모두 꺼져 lexical로만 돌던 상태(캐시 123장 잔존). 원인=pip 3.14 truststore 재귀로 fastembed 설치 실패(F단원). 해결=`pip install --use-deprecated=legacy-certs fastembed numpy pillow socksio` 후 `codex_image_embed.py selftest`(jina-clip-v1 ~1GB, 캐시 123→168 자동채움). **일러스트 매칭=이제 CLIP(codex_image_embed) 담당** → `run_codex_casual.bat:81` gemini 일러스트 태거(`codex_illust_tag.py`, 429 다발) **비활성화(주석)**; photo 태거(L80)는 유지. `SETUP_EMBED.bat`/`SETUP_FULL_PRODUCER.bat`에 legacy-certs 폴백 tier 추가.
...
- **★ pip 의존성 설치 (Python 3.14 truststore 재귀 우회, 2026-07-16)**: 런타임이 3.14면 pip의 vendored truststore가 `ssl.verify_mode`에서 무한재귀(RecursionError) → **모든 HTTPS 설치 실패**(fastembed·faster-whisper 등, `--isolated`로도 안 됨). 우회 = `python -m pip install --use-deprecated=legacy-certs ...`(pip `_internal/cli/index_command.py`: legacy-certs면 truststore ssl_context 안 만듦, 코드확정). 구버전 pip엔 플래그 없어 에러 → `SETUP_EMBED.bat`·`SETUP_FULL_PRODUCER.bat`은 **legacy-certs→plain→--user 3-tier 폴백**. 네이티브 확장은 3.14 wheel 여부 별도 확인(ctranslate2 4.8.1=cp314 O). `.phonespot_runtime`가 3.14 = 이 이슈 상시.
...
**2026-07-21 세션 — GA4 페이지별 퍼널 (리틀리/시티마켓 분리측정, GA4 전담 task / P4 구현)**
- 목적: GA4가 property 전체를 합산해 리틀리·시티마켓 성과가 뒤섞이던 문제(P4) 해결. **세션 시작 페이지(landingPage) 기준**으로 유입을 리틀리/시티마켓/기타로 분리. 리틀리 경유로 시티마켓 도착·카톡한 건은 리틀리 성과로 귀속.
- **신규 파일 `apps_script/page-funnel.js`** (GA4 전담 task 소유). 함수: `fetchPageFunnel`(GA4 runReport→집계→두 탭 재작성), `pf_fetch_`·`pf_buckets_`·`pf_classify_`·`pf_dateSet_`·`pf_sum_`·`pf_writeDetail_`·`pf_writeFunnel_`, `setupPageFunnelTrigger`(매일 03:30, 동명 트리거 삭제 후 재생성). 메뉴 = `Code.js` onOpen 메인 '브랜드 통합' > GA4 그룹 2줄(`fetchPageFunnel`/`setupPageFunnelTrigger`). 별도 상단메뉴는 오버플로로 안 보여 폐기.
- **신규 탭 2개**: `페이지별_퍼널`(요약 — 어제/7일/30일 × 리틀리/시티마켓/기타 × 방문·가격확인도착·카톡·전화·링크·카톡전환율 + 하단 '랜딩경로 상위' 분류 진단) / `GA4_페이지별`(날짜×유입×이벤트 일별). **★ 기존 GA4_자동·SUMIFS 소비수식·통합대시보드·리틀리 탭은 무수정**(신규 탭만 `insertSheet`, `insertColumnBefore` 미사용 = 컬럼시프트 사고 회피). push 전후 대시보드 채널별효율·리틀리 값 무변경 대조 확인.
- 분류: landingPage 경로 프리픽스. 기본 `/phonespot`→리틀리, `/pb,/poni,/pspot,/b2b`→시티마켓, `(not set)`/빈값→`PF_NOTSET_BUCKET`(기본 '기타'). **브랜드별 조정 = `_설정` 시트 키**(코드 push 불필요): `PF_A_NAME`/`PF_A_PATHS`/`PF_B_NAME`/`PF_B_PATHS`/`PF_NOTSET_BUCKET`. KT는 리틀리/시티마켓 구조 없어 대부분 '기타'(정상).
- 실측(2026-07-21): citymarket_arrival 유입경로 = meta 57.5%·(direct) 21.7%·daangn 10.9%·littly 4.2% → **source로는 경유/직접 구분 불가**(cross-domain 대체로 이어짐), landingPage로만 분리됨. 카톡클릭(`kakao_chat_click`)은 litt.ly 전용, `citymarket_arrival`은 citymarket 전용.
- **함정**: ①**6분 한도** — 차원은 `landingPage`(경로만) 써서 원천 행 최소화(`landingPagePlusQueryString`는 쿼리스트링 중복으로 6.4만 행→타임아웃; API가 landingPage 거부 시 자동 폴백). 시트 쓰기는 **탭당 setValues 1회 배치**(행별 setValues/setNote 금지). ②`pf_dateSet_`는 **날짜 문자열(yyyymmdd) 비교** — start/end를 각각 new Date()로 만들면 ms 차이로 타임스탬프 비교 시 마지막 날 탈락(30일 열 하루 누락 버그, 수정됨). ③**시티마켓 카톡·전화 = GTM 이벤트 태그(`kakao_chat_click`/`phone_click`) 없으면 0** — 시티마켓에 카톡 버튼 있으나 태깅 안 됨(사이트/GTM 작업, `GTM-TMXR6VL9`). ④**`(not set)` 랜딩** = 7/9 시티마켓 디자인 개편 후 랜딩 계측(page_view/page_path) 깨진 신호. 정황상 시티마켓 유입(citymarket_arrival만·카톡 0)이나 GA4가 랜딩을 못 잡음 → 근본해결은 GTM. ⑤끝 알림은 `getUi().alert` 금지(확인 클릭 대기=무한로딩처럼 보임) → `ss.toast` 논블로킹.
- 상태 갱신: **P1**(citymarket_arrival 7/3~7/13 0)은 **7/14 자가복구** 확인(단 배포전 500~870/일 → 복구후 140~290/일). **P3**(KT 오염)은 KT `_설정` `GA4_PROP_ID=543861861` 정상 반영으로 **오염 없음**(폰스팟 534396517 KT 시트에 0건). P2(fetchGA4Daily 전용트리거)는 미착수(현재 데이터는 정상). 인수인계 정본 = `_docs/GA4_TASK_시작프롬프트.md`.

**2026-07-27 세션 — 사전예약 랜딩(별도 GA4 속성) 통합 + 메타+ 유입 소스분기 (GA4 전담 task)**
- 배경: 메타 광고 일부가 리틀리(litt.ly)가 아닌 **별도 랜딩 `phonespot86.github.io/preorder`**로 착지. 이 랜딩은 **별도 GA4 속성 `폰스팟 사전예약`(546666245)**에 연결(본속성 534396517=리틀리/시티마켓과 데이터 완전 분리). ★종민 결정: **속성 병합 안 함**(리틀리=리틀리페이지 / 사전예약=웹 랜딩, 성격 다름) → 각 속성 별도 read.
- **신규 함수 `fetchPreorderGA4`**(`page-funnel.js`): `_설정 PREORDER_GA4_PROP_ID`(폰스팟=546666245, 미설정 브랜드=KT는 no-op) 읽어 GA4 runReport → 탭 **`사전예약_GA4`**(date/host/pagePath/campaign/event/eventCount/sessions) 재작성. **`fetchPageFunnel` 내부(03:30 트리거)에서 자동 호출** → 매일 갱신. 메뉴 🧾 사전예약 페이지 GA4 수집.
- **메타+ 유입 소스분기**(`meta-sync.js syncMetaCampaignIntegrated`, `hasPre` 게이트=`PREORDER_GA4_PROP_ID` 있을 때만 활성): **M(13) 열을 전화클릭→'유입' 라벨로 용도변경**(header setValue '유입'). 광고그룹별 유입 판정 = `VLOOKUP(광고그룹명 E, FILTER({UTM_GRP,UTM_INF}, UTM_CH="페북"), 2)` = **UTM 탭 G열(유입구분)** 값. 유입="사전예약" → K세션·L카톡을 **사전예약_GA4**(event `kakao_click`)에서, 그 외 → **GA4_자동**(본속성, event `kakao_chat_click`)에서 SUMIFS. N/O(시티마켓)는 유입=사전예약이면 0. **KT(hasPre=false)는 기존 동작 그대로=무회귀.**
- **★ 카톡클릭 이벤트명이 속성마다 다름**(핵심 함정): 본속성=`kakao_chat_click` / 사전예약=`kakao_click` / KT=`chat_click`(Code.js `normalizeGA4Event_`가 importGA4·page-funnel서 `chat_click`→`kakao_chat_click` 정규화). 그래서 유입 분기 시 속성별로 다른 이벤트명을 SUMIFS해야 함(하드코딩됨). `phone_click`·`citymarket_click`은 **어느 속성에도 존재 안 함**(실측 2026-07-27).
- **UTM G열(유입구분)**: named range `UTM_INF=G:G`(F열은 기존 메모라 충돌 회피 위해 G 사용). `ensureUtmNamedRanges_`가 셋업 시 **G1 '유입구분' 헤더+G열 노랑배경 자동생성**(2026-07-27). 값=리틀리/시티마켓/사전예약(공백=본속성 기본).
- **검증(2026-07-27, xlsx 실측)**: 사전예약_폴드8(배너·preorder)·랜딩페이지_폴드8(영상·meta_earlybird) 두 그룹 유입=사전예약, 카톡클릭이 사전예약_GA4 `kakao_click`값과 일치(배너≈82·영상≈60), 시티마켓=0. 리틀리 그룹은 유입 공백·본속성 값 무변경 = **교차오염 없음**. GA4_자동 이벤트 9종(실측): session_start/page_view/first_visit/user_engagement/scroll/**citymarket_arrival**/click/**kakao_chat_click**/price_check. → **N(시티마켓클릭=`citymarket_click`)=메타+ 379행 전부 0**(이벤트 부재), O(시티마켓직접=`citymarket_arrival`)=113행 유효(meta 소스 6,258건).
- **함정**: ①**중복 UTM 행**(같은 광고그룹명 E가 2슬러그) → VLOOKUP 첫 매치만 잡아 유입/슬러그 오판(예 `사전예약_폴드8`이 preorder·meta_earlybird 2행 존재 시) → 수기 삭제. ②사전예약 페이지엔 citymarket 이벤트 없음 → N/O=0(정상). ③`fetchPreorderGA4`는 `fetchPageFunnel`(03:30) 경유로만 호출 = refreshAll(02:45)엔 없음(리틀리 완전이전 시 refreshAll에도 추가 검토). ④**Script Property `PREORDER_GA4_PROP_ID=546666245`(폰스팟만)** = hasPre 스위치. KT엔 미설정이라 유입분기 자동 비활성.
- **★ 장기(종민 2026-07-27): 리틀리 폐지→랜딩 주력 전환 예정** → 시간이 갈수록 유입=사전예약 행이 다수·리틀리 행 소멸, 시티마켓 N·O 열도 소멸 예정. 전환 개편안(유입 기본값 플립 or 필수 드롭다운 검증 + 시티마켓 N·O 2열 동시 제거 + 유입→속성 매핑 테이블화)은 별도 검토. **컬럼시프트 위험**으로 N 단독 삭제는 보류하고 개편 때 일괄 처리 권장.
...
- 2026-07-06 (세션): **상단 메뉴 제목 앞 이모지 제거 (G단원, 메뉴바 폭 개선).** createMenu 10개 제목에서 선두 이모지+공백 제거: 통합(🚀)·메타(📘)·유튜브(🎥)·네이버(🟢)·당근(🟠)·구글(🔵)·광고그룹 추이(📊)·소재 인사이트(🎨)·알림/모니터링(🔔)·데이터 질문(🤖). 내부 addItem 이모지는 유지(폭 무관). 각 *-sync.js/Code.js/adgroup-trend.js/creative_insights.js/alerts.js/nl_query.js/youtube_sync.js.
- 2026-07-13 (세션): **멀티브랜드 상시적용 규칙(CLAUDE.md STEP 3) + 광고그룹추이 헤더 표기 통일.** ① 종민 지시: 모든 변경은 폰스팟+KT 둘 다 무조건 상시 적용, 브랜드 물어보지 말 것(코드는 push→clasp 양쪽 자동배포). ② adgroup-trend.js 헤더를 메타+ 표기와 통일: '카톡전환율'→'카톡전환률', '카톡당CPL'→'카톡당 CPL'(차트 라벨 포함). ★광고그룹 추이 헤더+데이터는 refreshAdgroupTrendChart가 코드생성(63행 헤더 setValues, 64행~ 데이터) — 수기 유지 불필요, '추이 갱신' 돌리면 코드가 세팅.
- 2026-07-13 (세션): **광고그룹 추이 드롭다운 정렬+필터 (A단원, adgroup-trend.js W60).** 문제: E61 광고그룹 드롭다운이 UNIQUE(FILTER())라 등장순(정렬X) + 죽은 광고그룹까지 전부 떠서 찾기 어려움. 수정: W60 수식을 `SORT(UNIQUE(FILTER(채널명열, 명<>"", 채널날짜A>=TODAY()-60)))`로 — 가나다 정렬 + 최근 60일 활성만(메타+/당근+/네이버+ 각 A열=날짜 기준). 60일 넘게 미집행 광고그룹은 목록에서 제외(옛것 보려면 -60을 늘림). 반영=메뉴 '광고그룹 추이 → 차트 셋업' 재실행(W60 수식 재설정, 토글값은 보존).
- 2026-07-13 (세션): **GA4 페이지별 수집 (E단원, Code.js fetchCitymarketPages).** 기존 GA4_자동은 source/campaign/event 차원만이라 시티마켓 페이지를 못 가려냄. 신규 함수: AnalyticsData runReport 차원 [date,hostName,pagePath,eventName] 메트릭 [eventCount,sessions] 최근30일 → 새 탭 'GA4_페이지'(날짜/호스트/페이지경로/이벤트/이벤트수/세션). 실행 시 호스트별 이벤트 집계 알림 → citymarket 호스트 있으면 시티마켓 페이지가 이 GA4에 추적됨(=표/분석 가능). 메뉴 🚀통합 '🌐 GA4 페이지별 수집(시티마켓 확인)'. ★용도: (A) 시티마켓 페이지 트래픽 표의 데이터 소스 + citymarket_arrival 7/3 고장 진단(시티마켓 페이지 자체 추적 여부 확인). 참고: citymarket_arrival만 7/2오후~7/3 0(사이트변경 추정), 다른 이벤트 정상.
- 2026-07-13 (세션): **광고그룹 추이에 (전체) 옵션 = 채널 일자별 통합성과 (A단원, adgroup-trend.js).** W60 드롭다운 수식 맨 위에 `={"(전체)"; SORT(UNIQUE(FILTER(...)))}` 로 '(전체)' prepend. refreshAdgroupTrendChart 필터: `if (adgroupName !== '(전체)' && name !== adgroupName) return;` → (전체) 선택 시 광고그룹명 필터 스킵하고 채널 전 광고그룹을 일자별 합산. 반영=메뉴 '광고그룹 추이 → 차트 셋업'(W60 수식 재설정) 후 토글 (전체) 선택 → 추이 갱신.
- 2026-07-13 (세션, 수정): **광고그룹 (전체) 드롭다운 — 배열리터럴 에러 회피.** 직전 `={"(전체)"; ...}` 배열리터럴이 로케일상 #ERROR! → **W60=값 '(전체)'(setValue) + W61=동적 리스트 수식(setFormula)** 로 분리. 드롭다운 requireValueInRange('W60:W500') 그대로라 (전체)+리스트 모두 선택 가능. 반영=차트 셋업 재실행.
- 2026-07-16 (세션): **광고그룹 추이 버그 2건 수정 (A단원, adgroup-trend.js).** ① **광고그룹 리스트 #ERROR! → 개별 선택 복구.** W61 동적 리스트 수식이 시트명 `메타+/당근+/네이버+`를 따옴표 없이 참조 → `+`가 덧셈 연산자로 파싱되어 setFormula 시 #ERROR!(개별 광고그룹 목록 사라지고 (전체)만 남음). 수정: 시트명 전부 따옴표 처리 `'메타+'!E2:E` 등(Code.js:968 SUMIFS와 동일 규칙). ★교훈: setFormula로 넣는 수식은 UI자동따옴표가 없으므로 특수문자(+) 시트명은 반드시 홑따옴표. 반영=메뉴 '광고그룹 추이 → 차트 셋업' 재실행. ② **당근 문의율 0.00% 고정 → 클릭 기준 폴백.** 문의율=문의수/카톡클릭인데 당근+ J(카톡클릭)은 미집계(항상 0)라 문의수>0이어도 문의율=0. 수정: `inquiryRate = 카톡클릭>0 ? 문의/카톡클릭 : (클릭>0 ? 문의/클릭 : 0)` — 메타·네이버는 정의(문의/카톡클릭) 유지, 당근만 문의/클릭 폴백. ★단서: 당근 문의율 분모=클릭(광고클릭)이라 메타·네이버 문의율(분모=카톡클릭)과 직접 비교 불가(당근은 카톡클릭 funnel 단계 없음). 당근 카톡전환률·카톡당CPL 컬럼도 같은 이유로 0(정상, 미집계).
- 2026-07-16 (세션): **통합대시보드 우측 열 폭 통일 (G단원, Code.js buildDashboardV2 폭 마무리 ~1171).** 기존 H(8)=100·I~M(9~13)=108 → **H~M(8~13) 전부 80 고정**(좌측 B~F=80과 통일). 최종 폭: A=100(라벨)·B~F=80·G=60(간격)·H~M=80. 종민 지시(H~L 요청 → M 고아 방지 위해 H~M 전체 80 확정). 반영=nightlyDashboard/refreshAll의 buildDashboardV2 재실행 또는 메뉴 재빌드(양쪽 브랜드 각각).
- 2026-07-16 (세션, 함정): **buildDashboardV2 수동 실행 시 "무한로딩"처럼 보이는 정상 동작 (G단원, Code.js:1190).** buildDashboardV2 맨 끝에 완료 알림 `getUi().alert("✅ 통합대시보드 V2 빌드 완료...")`가 try-catch 없이 있음. ★함정: Apps Script 에디터 Run 버튼으로 nightlyDashboard/buildDashboardV2를 수동 실행하면, 대시보드는 다 그려진 뒤 이 alert 확인창이 **스프레드시트 탭**에 뜨는데 에디터엔 안 보여서 실행이 확인 클릭 전까지 멈춤 → 에디터가 "실행 중..."을 무한 표시(=버그 아님, 확인창 대기). 해제=시트 탭에서 확인 클릭. 자동 트리거(03:00)는 UI 없어 alert가 예외로 무시됨(정상 완료). 종민 결정(2026-07-16): alert 유지(toast 전환 안 함). 다른 setup 함수 alert도 대부분 동일 패턴(수동 메뉴 실행 전제).

- 2026-07-16: **커버 이미지 제거 + 타이포 3변형 로테이션 (C단원).** `Cover.tsx` 히어로 이미지 삭제(그림 재사용 누적으로 커버 유사 문제) → 타이포·컬러만; `VARIANTS[3]`(화이트/피치/오렌지반전)를 slug 앞숫자%3으로 자동 로테이션(피드 다양성). 커버는 `run_codex_casual.bat`의 `render_cover.mjs`로 재생성(재렌더 필요).
...
- 2026-07-16 (세션): **GA4 전담 task 분리 + 인수인계 프롬프트 신설 (G단원).** 종민 결정: GA4 영역만 별도 task 분리(개발권한=코드수정+push, 소유영역=GA4 코드만). 정본 = `_docs/GA4_TASK_시작프롬프트.md` — 연결(AnalyticsData advanced service v1beta, appsscript.json:4-9, oauthScopes 미명시), PropertyID(getBrandConfig_ → _설정/ScriptProperty/폴백 534396517 Code.js:8, KT=543861861 문서만), 수집 4종(fetchGA4Daily 최근7일·fetchGA4Backfill·importGA4 Code.js:329-395·fetchCitymarketPages Code.js:131-168), 탭구조(GA4_자동 헤더4행/데이터5행 A~H, GA4_페이지 헤더1행/데이터2행 A~F), 소비처(채널SUMIFS·채널별효율 Code.js:944-975·리틀리퍼널 1129-1148·syncLitlyActions_ 718-775·추세차트 446-470), UTM매핑(appendUnmappedUtmFromGA4 meta-sync.js:387-444 MIN_SESS=3, flipMappedUtmStatus meta-sync.js:516), 검증법(Drive MCP·체크리스트6·node --check), 함정(컬럼시프트·6분한도·self-heal 3축·MIN_SESS 자동삭제금지·네이버슬러그불일치·당근 daangn), 우선과제 P1 citymarket_arrival 7/3이후 0 / P2 fetchGA4Daily 전용트리거 부재 / P3 KT GA4 미연결 폴백오염 / P4 페이지차원 부재. **문서-코드 불일치 2건**: SYSTEM_MAP:425 "GA4는 syncAll 내장"=틀림(refreshAll Code.js:256에서 호출), SYSTEM_MAP:705 flipMappedUtmStatus "Code.js:527"=실제 meta-sync.js:516. CLAUDE.md STEP 2에 GA4 라우팅 행 추가.
- 2026-07-21 (세션): **GA4 페이지별 퍼널 — 리틀리/시티마켓 분리측정 (G단원, P4 구현).** 신규 `apps_script/page-funnel.js`(`fetchPageFunnel`+헬퍼, 트리거 03:30) + 신규 탭 `페이지별_퍼널`·`GA4_페이지별`. landingPage 기준 유입 분리(리틀리/시티마켓/기타), 기존 GA4_자동·SUMIFS·대시보드·리틀리 무수정. `_설정` 키 `PF_A/B_NAME`·`PF_A/B_PATHS`·`PF_NOTSET_BUCKET`로 브랜드별 조정(코드 push 불필요). 함정: 6분한도→landingPage(경로만)+setValues 배치, pf_dateSet_ 날짜문자열 비교(마지막날 누락버그 수정), 시티마켓 카톡/전화는 GTM 태그 필요, (not set)=7/9 개편후 랜딩계측 깨짐, getUi().alert 금지→ss.toast. 상세 = G단원 "2026-07-21 세션".
- 2026-07-21 (세션): **promo_ai 품질·안전 룰 4종 + 실사적합도 판별 정본 (C단원).** 050 제작 중 종민 지적으로 확립 — ①조명 일체화(AI티 원인=인물만 스튜디오광→합성느낌, 생성 프롬프트로만 해결. 그레인·흔들림·색보정은 반례로 기록) ②슈퍼톤 mp3 앞뒤 무음패딩 제거+loudnorm -14(050 17.3→14.97초, -23.8→-18.7dB) ③**부메랑 금지**(동종업계 비난 프레임 폐기: 마지막물량·오늘만이가격·요금제유지 전부 우리도 해당 → 폭로형→교육형 전환, "실구매가"→"할부원금" 용어통일) ④산출물 덤프 5종 확정 시 즉시 삭제(1회 정리 약 1GB 회수, allow_cowork_file_delete로 직접 삭제)+완료보고에 폴더경로 필수. 신설 `shorts/promo_ai/LIVE_ACTION_FIT.md`(주제 단위 실사적합도 점수화, 주제 task가 수집 시 태깅). 050 최종=`out_promo_ai/050_meme_store_lies/050_checklist_v7_*.mp4`(14.97초, 체크법형).
- 2026-07-16 (세션): **통합대시보드 월별 보기 추가 (G단원, Code.js buildDashboardV2).** 종민 요청 "대시보드 월별로". 두 가지: ① 상단 L2 전역 기간 드롭다운에 **이번달/지난달** 추가(Code.js:885) — N2 시작=IFS로 이번달=DATE(Y,M,1)/지난달=DATE(Y,M-1,1), O2 종료=이번달 TODAY()(진행중)/지난달 EOMONTH(TODAY(),-1)(Code.js:887-889). L2 따라가는 요약스트립·채널별효율·실적매칭·SNS 전부 자동 반영. ② **월별 핵심 표 신설**(40~48행, 좌측 A~F): 최근 6개월(EDATE로 월롤오버, 최신월 위) × 광고비/문의/개통/CPL/순이익 — 기간별핵심과 동일지표를 달력월(1일~말일) 기준. 순이익 F42:F47 조건부서식(초록/빨강) 추가. 푸터 38→50행, hideRows 51~59. ★한계: 채널시트·GA4는 대체로 최근30일치라 현재는 6·7월만 실값, 이전 달은 0(데이터 쌓이면 자동). 반영=대시보드 재빌드(nightlyDashboard/메뉴, 폰스팟+KT 각각).
- 2026-07-16 (세션): **대시보드 특정월 선택기 추가 (G단원, Code.js buildDashboardV2).** 종민 요청 "이번달/지난달 옆에 월 그냥 지정". L2 기간 드롭다운에 "특정월" 옵션 + **M2에 월 드롭다운(1~12, 노란칸, 라벨 M1="월지정", numberFormat 0"월")** 신설(Code.js:885-892). L2="특정월"이면 N2 시작=DATE(YEAR(TODAY()),$M$2,1), O2 종료=MIN(EOMONTH(그달,0),TODAY())(당월이면 오늘까지). M2 기본값=현재월(빈칸일 때만, 재빌드마다 L2처럼 리셋). 요약스트립·채널별효율·실적매칭 전부 N2/O2 따라가 자동 반영. ★한계: 당해년도 기준(월>현재월이면 미래=빈값), 채널/GA4 최근30일치라 과거월은 데이터 범위 밖이면 0. 반영=buildDashboardV2 재빌드(전체새로고침 refreshAll 쓰지 말 것—6분초과, nightlyDashboard 자동 또는 에디터 buildDashboardV2 직접실행).
- 2026-07-16 (세션, 버그수정): **특정월 M2 드롭다운 검증 위반 수정 (G단원, Code.js:890).** M2에 numberFormat `0"월"` 걸었더니 드롭다운이 표시값 "7월"(텍스트)로 저장→requireValueInList([1..12] 숫자) 위반으로 튕김(클릭 시 "1~12 입력" 에러). 수정: numberFormat `0"월"`→`0`(숫자 그대로). 라벨 M1="월지정"이 월 의미 전달. ★교훈: requireValueInList(숫자) 셀에 접미문자 numberFormat 걸면 드롭다운이 포맷문자열을 저장해 검증 위반. 임시회피=숫자 직접 타이핑(7). 반영=buildDashboardV2 재빌드.
