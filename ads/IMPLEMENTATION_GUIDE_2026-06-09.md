...
- **영속화**: localStorage `phonespot_brand_profiles`(이름→프로필 map) + `phonespot_active_profile`. ☁시트 백업/⬇불러오기 = Code.js `pushBrandProfilesToSheet`/`getBrandProfilesFromSheet` (시트 `브랜드_설정` A1=JSON·B1=시각, 자동 생성. Code.js ~1461). **★ 소유권(2026-07-02): 이 2함수 + `SHEET_BRAND_PROFILES`/`getBrandProfilesSheet_`는 물리적으로 Code.js에 있으나 generator 기능 소유 — 다른(Code.js) task는 삭제·이동 ❌. 신규 서버함수 필요 시 종민 확인 후 추가.**
