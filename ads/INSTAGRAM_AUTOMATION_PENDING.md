# 인스타 시트 자동화 — ✅ 완료 (2026-06-11)

> **상태: 2026-06-11 완료.** `meta-sync.gs` 내 `syncInstagramDaily()` + 매일 02:00 자동 트리거.
> - **INSTAGRAM_BUSINESS_ID**: `17841474706647015` (@phonespot.kr / 팔로워 49 / 게시물 83)
> - **권한**: 시스템 사용자 토큰 신규 발급 시 `instagram_basic` + `instagram_manage_insights` 포함. (기존 토큰에 권한 자동 추가 ❌ — 토큰 발급 시점에 scopes 확정)
> - **시트 컬럼**: A날짜 / B포맷 / C주제 / D permalink / E조회수 / F좋아요 / G팔로워 (H~I 수동)
> - **매칭 키**: D열 permalink — 매칭 시 E·F·G만 갱신 (A~D 보존)
> - **정렬**: `posts.sort` timestamp 오름차순 → 최신이 시트 하단
> - **2024+ 통합 메트릭 `views`** 우선 시도 → 폴백: `plays` / `video_views` / `impressions`
> - **메뉴**: `📷 인스타 동기화` (수동 테스트용)
