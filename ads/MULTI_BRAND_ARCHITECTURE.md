...
### 빈 템플릿화 잔재 보강 (2026-06-30)
- `clearBrandDataForTemplate` 1차 실행 후 폰스팟 데이터가 남은 4곳 발견(Drive 직접 read로 확인) → 함수에 3곳 추가:
  - `['통합대시보드', 64, 10]` — 광고그룹 추이 데이터(60행~)는 buildDashboardV2가 안 건드려서 폰스팟 리틀리 데이터 잔존 → 64행~ 비움.
  - `['문의접수', 2, 8, 11]` — 우측 카카오 친구수 블록(H~K) 폰스팟 데이터. **4원소 [name,행,시작열,끝열] 포맷 신설**(중간 열범위 클리어 지원).
  - `['유튜브_인사이트', 2, 6]` — 폰스팟 영상 분석 결과(재생성됨).
- 루프가 `t.length===4`면 [시작열,끝열], 아니면 [1,끝열]로 클리어.
- **📌참조 탭은 유지**(스킵): KT폰샵 자체 정보(URL kt-phoneshop.com/사업자 그로우플랜)+전 브랜드 서비스 목록이 든 공용 참조표라 비우면 KT 정보도 소실 → 폰스팟 전용 행만 수동 정리 권장.
- **★ 클로드의 KT 시트 점검 = Drive 직접 read**(get_file_metadata/read_file_content). 폰스팟은 일일 스냅샷(03시) 있으나 KT는 스냅샷 대상 아님 → KT 커지면 apps_script_sheet_export에 KT 추가 검토(현재는 직접 read로 충분).
- KT 빈 템플릿화 완료(2026-06-30). 남은 일: KT GA4 생성→속성ID 입력→웹사이트 GA4/GTM→GA4 sync·트리거 ON / KT 메타·네이버 계정 가입→토큰 Script Property.

---

## ★ Meta 멀티브랜드 표준: "허브 BM" 중앙집중 모델 (2026-07-03 확정·검증)

브랜드가 계속 늘어나는 전제. 브랜드 추가가 매번 동일 3스텝이 되도록 Meta 접근을 중앙집중한다.

### 구조
- **폰스팟 BM = 허브.** 앱 1개(`phonespot-internal-sync`) + sync 시스템유저 1개(**`phonespot-sync`**) + **장수명 토큰 1개**.
- 각 브랜드 광고계정 → 허브 BM에 **파트너 공유** + `phonespot-sync`에 **자산 할당**.
- 각 브랜드 시트 → 자기 `META_AD_ACCOUNT_ID` + **같은 `META_TOKEN`**(허브 토큰 복사). 코드(`getToken()`/`getAdAccountId()`)가 시트별 Script Property를 읽으므로 **코드 수정 0**.

### 왜 이 모델인가 (검증된 근거)
- **토큰 발급 단계를 영구 회피.** 신규 시스템유저로 토큰 생성 시 "앱에 부여할 권한 없음(사용 가능한 권한 없음)"이 뜬다 — 앱이 그 유저에게 ads_read scope를 노출 안 하기 때문(앱 레벨). 허브 토큰 재사용이면 이 지옥을 안 거친다.
- **시스템유저 토큰은 발급 후 자산을 추가해도 그 자산에 접근된다** → 브랜드 계정을 `phonespot-sync`에 할당만 하면 기존 토큰이 즉시 그 계정도 읽음(재발급 불필요).
- 데이터는 시트별 `META_AD_ACCOUNT_ID`로만 수집 → 브랜드 간 안 섞임.
- 관리 토큰 1개(N개 아님) → 확장할수록 단순.
- 관리자 시스템유저 한도 = BM당 1명(기본 tier). 그래서 브랜드마다 관리자 유저를 못 만든다(=허브 1개가 정답). 굳이 브랜드별 유저 쓰려면 "직원(Employee)" 역할 + 앱에 역할 부여가 필요하고, 토큰 scope 문제까지 겹쳐 비효율.

### 트레이드오프 (정직)
토큰 1개 공유 → 재발급 시 전 브랜드 시트 갱신 필요. 단 시스템유저 토큰은 만료 없어 재발급이 드묾. **완전 토큰 격리를 원하면** 앱에 Marketing API 고급액세스를 1회 세팅해야 브랜드별 토큰 발급이 열린다(앱 작업 1회 추가). 확장 편의 우선이면 허브 모델.

### 📋 브랜드 추가 표준 체크리스트 (반복용)
**Meta (허브=폰스팟 BM):**
1. 신규 브랜드 광고계정 → 허브 BM에 **파트너 공유** (KT 사례: KT BM에서 폰스팟 BM 비즈니스ID로 광고계정 파트너 할당 → 폰스팟 목록에 "○○에서 소유" 뜨며 즉시 접근, 별도 승인 대기 없음)
2. `phonespot-sync` 시스템유저 → 할당된 자산 → 광고 계정 추가 → 그 계정 → "분석 보기"

**시트:**
3. `🚀 통합 → 🏢 _설정 탭 생성/갱신` 후 값: `BRAND_NAME` / `GA4_PROP_ID`(브랜드 GA4 속성ID=오른쪽 "속성 및 앱"의 숫자, 계정ID 아님) / `CARRIER_FILTER_MODE=none`(전용계정 전부수집) / `INSIGHTS_DRIVE_FOLDER`(브랜드별 분리) / `TARGET_CPL`
4. Script Property(시크릿): `META_TOKEN`=폰스팟 시트 값 그대로 복사 / `META_AD_ACCOUNT_ID`=`act_`+브랜드계정숫자 / (쓰면) `NAVER_API_LICENSE`·`NAVER_SECRET_KEY`·`NAVER_CUSTOMER_ID`
5. 트리거: `🚀 통합 → ⏰ 야간 전체 새로고침(02:45)` + 편집기 `setupNightlyDashboardTrigger`(03:00) + `🟢 네이버 ⏰ Daily`(쓰면) + `📘 메타 ⏰ Daily`+`오후 재sync(14:00)`
6. `📘 메타 🔑 연결테스트` → `🚀 통합 ⚡ 전체 새로고침` → `📏 날짜(A열) 폭 80`

**동시반영 원리:** 코드=git push 1회→Actions clasp가 전 브랜드 scriptId 배포. 데이터·대시보드=각 시트 트리거가 각자 자기 _설정·토큰으로 매일 갱신. "한 스크립트가 다른 시트 데이터 대신 수집"은 멀티브랜드 격리상 불가 → 각자 트리거가 정답.

### GA4 참고
- GA4는 브랜드별 속성 필요(GA4_PROP_ID 각자). 폰스팟=534396517, KT폰샵=543861861.
- 실행계정(313jongmin@gmail.com)이 각 GA4 속성에 뷰어 이상 권한 있어야 수집.
- UTM 탭은 GA4 수집이 아니라 **메타/네이버 광고그룹 sync가 처음 돌 때 자동 생성**(autoDiscoverAdsets_). 메타 미연결이면 UTM 없음이 정상.
