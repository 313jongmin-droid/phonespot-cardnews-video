...
- **★ 어색함 방지 하드룰 (2026-07-09, 042 점검발 — 대본이 "미묘하게 어색"한 원인)**:
  1. **비트 끝 군더더기 문장 금지** — 정보 뒤 hedge·요약 마무리 삭제. ❌ "아직 공식 발표 전이라 전망입니다"(앞에서 이미 hedge) · "급한지 아닌지에 따라 갈립니다"(앞 문장 요약) · "~로 읽힙니다/~로 보입니다"(분석체). 팩트 마지막 문장은 **단정 또는 행동유도**로 끝낸다("~겁니다", "~하세요").
  2. **전문용어·영문 기능명 금지** — "폼팩터"→"접는 폰" 식으로 한글 구체어. 스펙/UX/폼팩터/카테고리 류 금지(§카드 규칙 영문 최소화와 동일, 하드룰로 격상).
  3. **같은 어미·부사 반복 금지** — "~전망입니다"를 연속 카드에 반복 ❌, "그동안" 등 시작어·어미 단조 반복 ❌. 카드마다 어미·시작어를 바꿔 리듬을 준다.
  4. **훅(card_1)은 요약이 아니라 첫 문장부터 후킹** — "~챙겨볼 때입니다" 같은 마무리 톤 ❌. 첫 1초에 궁금증·반전.
  5. **자가검증**: 각 body를 소리내어 읽어 "AI가 쓴 듯 밋밋한 마무리 문장"이 있으면 잘라낸다. 6번 §6 체크리스트에 포함.
