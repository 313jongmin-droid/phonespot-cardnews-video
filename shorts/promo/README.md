...
```
- 결과: `out/promo/NNN_이름_프리셋.mp4`. **재렌더 시 덮어쓰지 않고 `_1`,`_2`… 자동 넘버링**(기존 파일 보존). **효과음+음악 기반, 나레이션 없음.**
...
- 출력: `out/promo/`만(upload 복사 폐기). 재렌더는 `_1`,`_2` 자동 넘버링(덮어쓰기 X) — bat의 `:uniq`/`:nextname`. 나레이션 제거 — 효과음+음악만.
