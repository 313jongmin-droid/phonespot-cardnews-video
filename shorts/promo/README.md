# 폰스팟 홍보영상(promo) 트랙 — 정본 마스터 가이드 (현행 2026-06-13)
...
│            promo_review.py · promo_learn.py · promo_merge_brand.py · promo_pick_music.py
└─ src/components/promo/
   ├─ PromoShort.tsx        (디스패처: 섹션별 스타일/효과음 + 음악(music_src/music_start) + 고정 타이밍 + 비네트)
   └─ styles/
      ├─ shared.tsx (팔레트 PREMIUM·VARIANTS·CtaBlock·헬퍼·PromoStyle·BasicOpening/Outro)
...
규칙: `화면` 짧게 · 마크다운 에디터가 `-`를 `*`로 바꿔도 인식됨.
...
- 타이밍 고정(섹션 기본 2.4s · CTA 3.0s, 스크립트 `dur`(초)로 조정) — `Root.tsx`의 `calcPromo`/`promoSeconds`.
...
- 출력 단일화: `out/promo/`만(upload 복사 폐기). 나레이션 제거 — 효과음+음악만.
