...
- ★★ **최종 확정 시 덤프 전량 삭제** (종민 2026-07-21, 필수 절차): 영상이 확정되면 **아래 5종을 그 자리에서 삭제**한다. 안 하면 계속 쌓인다(2026-07-21 1회 정리에서 **약 1GB** 회수).
  1. **sandbox 스크래치** `/mnt/outputs/b<slug>*` — 세그먼트·wav·ass 전부(최종본은 out_promo_ai에 있으므로 불필요). 실측 812MB.
  2. **검증 프레임 폴더** `_chk*` `_inv*` `_rev*` `_insp*` `_ref_tmp` `_analysis*` — 전부.
  3. **폐기본 mp4** — 방향이 뒤집혀 안 쓸 버전(050의 폭로형 v1~v3 등)은 혼동 유발하므로 **삭제**.
  4. **구버전 mp4** — 최종본 1개만 남기고 나머지 삭제(보존 필요 시 `_archive/`).
  5. **나레이션 임시 mp3** `assets/audio/narration/2026-*.mp3` — 슈퍼톤이 날짜명으로 떨구는 원본. **표준명 `<NNN>_n<N>.mp3`로 복사한 뒤 날짜형은 삭제**.
  ※ `rm`이 `Operation not permitted`로 막히면 **`mcp__cowork__allow_cowork_file_delete`를 호출해 권한을 받고 삭제**한다. "종민이 탐색기에서 지워라"로 떠넘기지 말 것(구 지침 폐기).
- ★ **완료 보고 시 폴더 경로 필수** (종민 2026-07-21): 영상 1편이 최종 완료되면 **① 최종 파일 present + ② 산출물 폴더 전체 경로(Windows) + ③ 폴더 내 파일 목록(최종본/구버전/캡션 구분)**을 항상 같이 준다. 파일 링크만 주면 종민이 폴더를 직접 찾아가야 하므로 ❌. 형식 예:
  `폴더: C:\backup\phonespot_cardnews\shorts\promo_ai\out_promo_ai\<slug>\` + 그 안 파일 목록 표(최종본 표시).
  ※ 구버전(`_v1`~)이 쌓여 있으면 **`_archive/`로 정리하거나 삭제 대상**을 함께 안내(탐색기 작업은 종민이).
