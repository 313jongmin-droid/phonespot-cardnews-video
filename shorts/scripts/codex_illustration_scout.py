...
STYLE = """[핵심 지침 — 가장 먼저 지킬 것]
1) 고급스럽게: 세련되고 완성도 높은 프리미엄 퀄리티로 그릴 것.
2) 범용성 보장(디테일 묘사 X): 한 기사에만 맞는 구체 디테일(특정 모델명·날짜·수치·1회성 장면)을 그리지 말고, 여러 영상에서 재사용 가능한 '일반 개념'만 표현할 것.
3) 개별 파일 생성: 각 항목을 개별 파일로 하나씩 따로 생성할 것(여러 개를 한 장에 모아 그리지 말 것).

한국 휴대폰·IT 뉴스 쇼츠용 고품질 에디토리얼 일러스트.
