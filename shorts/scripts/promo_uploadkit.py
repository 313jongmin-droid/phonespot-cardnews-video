...
# "복붙해서 같이 업로드" 용. 결과: out/promo/<slug>_upload/ 안에 영상 + 텍스트 동거.
# usage: promo_uploadkit.py <nn> [outfile.mp4]
#   - outfile 주면 그 영상 사용, 없으면 out/promo/<slug>_*.mp4 중 최신.
...
if outfile_arg and os.path.exists(outfile_arg):
    ...
    vids = sorted(glob.glob(f"out/promo/{slug}_*.mp4"), key=os.path.getmtime)
    mp4 = vids[-1] if vids else ""

dest = f"out/promo/{slug}_upload"
